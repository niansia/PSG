...
### ルーティングだけではトークンが減るとは限らない理由

PSG がモデルの文脈を減らすのは、ルーティングされた文脈が冗長なリポジトリ読み取りを**置き換えた**ときだけです。保存されたトレースでは、エージェントが PSG のルーティング結果を受け取ったうえで、なお従来どおりの探索を行っていました。リポジトリ全体の列挙、PSG の可搬状態や設定を通常の文脈として読むこと、そして関連シンボルがすでに特定された後にソースファイル全体を読み直すことです。

その場合、コストは置換ではなく加算になります。

```text
PSG ルーティング + 従来型の広い探索          ← トレースが示した実態
PSG ルーティング + 必要なときだけの補足読み   ← 本来あるべき姿
```

現在の Skill は PSG を主要な検索経路として扱います。Task Contract とルーティングされたシンボルから始め、十分な最小限のコード領域を読み、安全な変更に証拠が足りないときだけ範囲を広げます。ファイル全体の読み取りは許容されたフォールバックのままです。正確さはトークン節約より優先されるからです。

**この統合修正がトークンに与える影響はまだ再測定していないため、PSG はそれによるトークン削減を主張しません。**
