...
PSG normalizes authority into four tiers: `CLAIMED`, `RUNTIME_ATTESTED`, `USER_APPROVED`, and reserved `EXTERNAL_ATTESTED`. Ordinary MCP input is always `CLAIMED`, regardless of a caller-supplied source or actor label. Runtime checks can create `RUNTIME_ATTESTED`; separate local user actions can create `USER_APPROVED`. No authenticated external attestation adapter ships in v1, so an `external_tool` string remains claimed.
